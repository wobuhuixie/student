<template>
  <div style="background-color: #fff; border-radius: 8px;">
    ...
    <div class="filter">
      ...
      <el-dialog    
          ...
          <el-form ref="form" :model="form" :rules="rules" label-width="100px" class="demo-ruleForm" style="overflow: hidden;">
              <el-form-item label="工号" prop="infonum"  required>
                  <el-input placeholder="请输入工号,格式为:xx01" v-model="form.infonum"></el-input>
              </el-form-item>
      ...
      <el-dialog    
          ...
          <el-form ref="updform" :model="updform" :rules="updrules" label-width="100px" class="demo-ruleForm" style="overflow: hidden;">
              <el-form-item label="工号" prop="updinfonum">
                  <el-input placeholder="请输入工号,格式为:xx01" v-model="updform.updinfonum"></el-input>
              </el-form-item>

              <el-form-item label="姓名" prop="updname" >
                  <el-input placeholder="请输入姓名" v-model="updform.updname"></el-input>
              </el-form-item>

              <el-form-item label="年龄" prop="updage" >
                  <el-input placeholder="请输入年龄" v-model="updform.updage"></el-input>
              </el-form-item>

              <el-form-item label="所在系" prop="upddepartment">
              ...
              <el-form-item label="学历" prop="upddegree"  >
              ...
              <el-form-item label="职务" prop="updoffice">
               ...
               <el-form-item label="性别" prop="updgender">
    ...
    <div class="box">
      ...
      <!-- <el-button 
        type="primary" 
        @click="onAdd('infotabledata','Information')" 
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button
            type="danger"
            @click.native.prevent="deleteRow(scope.row.infonum, fulltabledata)"
            icon="el-icon-delete"
            size="mini">
            删除
          </el-button>
          ...
          <el-button 
            type="primary" 
            @click="onupd(scope.row.infonum,fulltabledata)"
    ...
    return {
      ...
      deleteRow(infonum){   //删除操作
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          const index = this.fulltabledata.findIndex(item => item.infonum === infonum)
          if (index !== -1) {
            this.fulltabledata.splice(index, 1)
            localStorage.setItem('infotabledata', JSON.stringify(this.fulltabledata))
          }
      ...
      onupd(upddata){    //展开修改信息
        ...
        const index = this.fulltabledata.find(item => {
          return item.infonum == upddata
        })
        this.lastinfonum = index.infonum
        this.updform.id = index.id
        this.updform.updinfonum = index.infonum
      ...
      addForm(formName){  //添加信息
        ...
        this.$refs[formName].validate((valid) => {    
          ...
          if (valid) {
            ...
            if(this.found){   //工号判断
              this.fulltabledata = JSON.parse(localStorage.getItem('infotabledata'))

              if(this.form.infonum.substring(0,2) === "xx" && !isNaN(this.form.infonum.substring(2,4)) ){

                  if(this.found){
                    this.fulltabledata.forEach((v) => {
                    if(this.form.infonum == v.infonum){
      ...
      updForm(){    //修改信息
        ...
        if(this.found){
          if(this.updform.updinfonum.substring(0,2) === "xx" && !isNaN(this.updform.updinfonum.substring(2,4)) ){
              

            if(this.found){
              if(this.lastinfonum == this.updform.updinfonum){
                  this.found = true
              }else{
                  this.fulltabledata.forEach((v) => {
                    if(this.updform.updinfonum == v.infonum){
                        this.$message.error("有相同工号，请检查是否输入正确！")
                        this.found = false
                      }
                    })
          ...
          if(this.found){
            if(this.updform.updname == ""){
                this.$message.error("姓名不能为空！")
                this.found = false
              }
          }
          if(this.found){
            if(this.updform.updage < 18 || this.updform.updage > 65){
              this.$message.error("年龄可能不合适？")
              this.found = false
            }
          }
        }
        if(this.found){         //found一路畅通为true 则执行修改信息操作
            this.fulltabledata.forEach((v,i) =>{
              if(v.id==this.updform.id){
                v.infonum =  this.updform.updinfonum
      ...
      getUser(){
         ...
         if (localStorage.getItem('infotabledata')) {
            this.fulltabledata = JSON.parse(localStorage.getItem('infotabledata')) || []
         } else {
         // 如果 localStorage 中没有数据，则将当前数据写入 localStorage
             localStorage.setItem('infotabledata', JSON.stringify(this.tableData))
             this.fulltabledata = this.tableData
          }

        // this.fulltabledata = localStorage.getItem('infotabledata') ? JSON.parse(localStorage.getItem('infotabledata')) : [];
        // if (!localStorage.getItem('infotabledata')) {
        //   localStorage.setItem('infotabledata', JSON.stringify(this.tableData));
        //   this.fulltabledata = this.tableData;
        // }
