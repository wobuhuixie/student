      ...
      <div class="filter">
        ...
        <el-dialog    
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="100px" class="demo-ruleForm" style="overflow: hidden;">
                <el-form-item label="工号" prop="finnum"   required>
                    <el-input placeholder="请输入工号,格式为:jm01" v-model="form.finnum"></el-input>
                </el-form-item>
        ...
        <el-dialog    
            ...
            <el-form ref="updform" :model="updform" :rules="updrules" label-width="100px" class="demo-ruleForm" style="overflow: hidden;">
                <el-form-item label="工号" prop="finnum"   updrequired>
                    <el-input placeholder="请输入工号,格式为:jm01" v-model="updform.updfinnum"></el-input>
                </el-form-item>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              type="danger"
              @click.native.prevent="deleteRow(scope.row.finnum, fulltabledata)"
              icon="el-icon-delete"
              size="mini">
              删除
            </el-button>
            ...
            <el-button 
              type="primary" 
              @click="onupd(scope.row.finnum,fulltabledata)"
      ...
      return {
        ...
        deleteRow(finnum){   //删除操作
          this.$confirm('此操作将永久删除, 是否继续?', '提示', {
            ...
            const index = this.fulltabledata.findIndex(item => item.finnum === finnum)
            if (index !== -1) {
              this.fulltabledata.splice(index, 1)
              localStorage.setItem('fintabledata', JSON.stringify(this.fulltabledata))
            }
        ...
        addForm(formName){  //添加信息
          ...
          this.$refs[formName].validate((valid) => {    
            ...
            if (valid) {
              ...
              if(this.found){   //工号判断
                this.fulltabledata = JSON.parse(localStorage.getItem('fintabledata'))
  
                if(this.form.finnum.substring(0,2) === "jm" && !isNaN(this.form.finnum.substring(2,4)) ){
  
                    if(this.found){
                      this.fulltabledata.forEach((v) => {
                      if(this.form.finnum == v.finnum){
        ...
        updForm(){    //修改信息
          ...
          this.fulltabledata.forEach((v,i) =>{
              if(v.id==this.updform.id){
                  v.finnum =  this.updform.updfinnum
        ...
        getUser(){
           ...
           if (localStorage.getItem('fintabledata')) {
              this.fulltabledata = JSON.parse(localStorage.getItem('fintabledata'))
           } else {
           // 如果 localStorage 中没有数据，则将当前数据写入 localStorage
               localStorage.setItem('fintabledata', JSON.stringify(this.tableData))
