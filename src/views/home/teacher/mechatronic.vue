      ...
      <div class="filter">
        ...
        <el-dialog    
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="100px" class="demo-ruleForm" style="overflow: hidden;">
                <el-form-item label="工号" prop="mecnum"   required>
                    <el-input placeholder="请输入工号,格式为:jd01" v-model="form.mecnum"></el-input>
                </el-form-item>
        ...
        <el-dialog    
            ...
            <el-form ref="updform" :model="updform" :rules="updrules" label-width="100px" class="demo-ruleForm" style="overflow: hidden;">
                <el-form-item label="工号" prop="mecnum"   updrequired>
                    <el-input placeholder="请输入工号,格式为:jd01" v-model="updform.updmecnum"></el-input>
                </el-form-item>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              type="danger"
              @click.native.prevent="deleteRow(scope.row.mecnum, fulltabledata)"
              icon="el-icon-delete"
              size="mini">
              删除
            </el-button>
            ...
            <el-button 
              type="primary" 
              @click="onupd(scope.row.mecnum,fulltabledata)"
      ...
      return {
        ...
        deleteRow(mecnum){   //删除操作
          this.$confirm('此操作将永久删除, 是否继续?', '提示', {
            ...
            const index = this.fulltabledata.findIndex(item => item.mecnum === mecnum)
            if (index !== -1) {
              this.fulltabledata.splice(index, 1)
              localStorage.setItem('mectabledata', JSON.stringify(this.fulltabledata))
            }
        ...
        addForm(formName){  //添加信息
          ...
          this.$refs[formName].validate((valid) => {    
            ...
            if (valid) {
              ...
              if(this.found){   //工号判断
                this.fulltabledata = JSON.parse(localStorage.getItem('mectabledata'))
  
                if(this.form.mecnum.substring(0,2) === "jd" && !isNaN(this.form.mecnum.substring(2,4)) ){
  
                    if(this.found){
                      this.fulltabledata.forEach((v) => {
                      if(this.form.mecnum == v.mecnum){
        ...
        updForm(){    //修改信息
          ...
          this.fulltabledata.forEach((v,i) =>{
              if(v.id==this.updform.id){
                  v.mecnum =  this.updform.updmecnum
        ...
        getUser(){
           ...
           if (localStorage.getItem('mectabledata')) {
              this.fulltabledata = JSON.parse(localStorage.getItem('mectabledata'))
           } else {
           // 如果 localStorage 中没有数据，则将当前数据写入 localStorage
               localStorage.setItem('mectabledata', JSON.stringify(this.tableData))
