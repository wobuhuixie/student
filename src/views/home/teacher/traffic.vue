      ...
      <div class="filter">
        ...
        <el-dialog    
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="100px" class="demo-ruleForm" style="overflow: hidden;">
                <el-form-item label="工号" prop="tranum"   required>
                    <el-input placeholder="请输入工号,格式为:jt01" v-model="form.tranum"></el-input>
                </el-form-item>
        ...
        <el-dialog    
            ...
            <el-form ref="updform" :model="updform" :rules="updrules" label-width="100px" class="demo-ruleForm" style="overflow: hidden;">
                <el-form-item label="工号" prop="tranum"   updrequired>
                    <el-input placeholder="请输入工号,格式为:jt01" v-model="updform.updtranum"></el-input>
                </el-form-item>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              type="danger"
              @click.native.prevent="deleteRow(scope.row.tranum, fulltabledata)"
              icon="el-icon-delete"
              size="mini">
              删除
            </el-button>
            ...
            <el-button 
              type="primary" 
              @click="onupd(scope.row.tranum,fulltabledata)"
      ...
      return {
        ...
        deleteRow(tranum){   //删除操作
          this.$confirm('此操作将永久删除, 是否继续?', '提示', {
            ...
            const index = this.fulltabledata.findIndex(item => item.tranum === tranum)
            if (index !== -1) {
              this.fulltabledata.splice(index, 1)
              localStorage.setItem('tratabledata', JSON.stringify(this.fulltabledata))
            }
        ...
        onupd(upddata){    //展开修改信息
          const index = this.fulltabledata.find(item => {
            return item.tranum == upddata
          })
          this.updform.id = index.id
          this.updform.updtranum = index.tranum
        ...
        addForm(formName){  //添加信息
          ...
          this.$refs[formName].validate((valid) => {    
            ...
            if (valid) {
              ...
              if(this.found){   //工号判断
                this.fulltabledata = JSON.parse(localStorage.getItem('tratabledata'))
  
                if(this.form.tranum.substring(0,2) === "jt" && !isNaN(this.form.tranum.substring(2,4)) ){
  
                    if(this.found){
                      this.fulltabledata.forEach((v) => {
                      if(this.form.tranum == v.tranum){
        ...
        updForm(){    //修改信息
          ...
          this.fulltabledata.forEach((v,i) =>{
              if(v.id==this.updform.id){
                  v.tranum =  this.updform.updtranum
        ...
        getUser(){
           ...
           if (localStorage.getItem('tratabledata')) {
              this.fulltabledata = JSON.parse(localStorage.getItem('tratabledata'))
           } else {
           // 如果 localStorage 中没有数据，则将当前数据写入 localStorage
               localStorage.setItem('tratabledata', JSON.stringify(this.tableData))
