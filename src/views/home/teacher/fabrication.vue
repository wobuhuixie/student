<template>
    ...
    <div style="background-color: #fff; border-radius: 8px;">
      ...
      <div class="filter">
        ...
        <el-dialog    
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="100px" class="demo-ruleForm" style="overflow: hidden;">
                <el-form-item label="工号" prop="fabnum"   required>
                    <el-input placeholder="请输入工号,格式为:zz01" v-model="form.fabnum"></el-input>
                </el-form-item>
        ...
        <el-dialog    
            ...
            <el-form ref="updform" :model="updform" :rules="updrules" label-width="100px" class="demo-ruleForm" style="overflow: hidden;">
                <el-form-item label="工号" prop="fabnum"   updrequired>
                    <el-input placeholder="请输入工号,格式为:zz01" v-model="updform.updfabnum"></el-input>
                </el-form-item>
      ...
      <div class="box">
        ...
        <!-- <el-button   
          type="primary" 
          @click="onAdd('fabtabledata','fabrication')" 
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              type="danger"
              @click.native.prevent="deleteRow(scope.row.fabnum, fulltabledata)"
              icon="el-icon-delete"
              size="mini">
              删除
            </el-button>
            ...
            <el-button 
              type="primary" 
              @click="onupd(scope.row.fabnum,fulltabledata)"
    ...
    data() {
      return {
        ...
        deleteRow(fabnum){   //删除操作
          this.$confirm('此操作将永久删除, 是否继续?', '提示', {
            ...
            const index = this.fulltabledata.findIndex(item => item.fabnum === fabnum)
            if (index !== -1) {
              this.fulltabledata.splice(index, 1)
              localStorage.setItem('fabtabledata', JSON.stringify(this.fulltabledata))
            }
      ...
      onupd(upddata){    //展开修改信息
        ...
        addForm(formName){  //添加信息
          ...
          this.$refs[formName].validate((valid) => {    
            ...
            if (valid) {
              ...
              if(this.found){   //工号判断
                this.fulltabledata = JSON.parse(localStorage.getItem('fabtabledata'))
  
                if(this.form.fabnum.substring(0,2) === "zz" && !isNaN(this.form.fabnum.substring(2,4)) ){
  
                    if(this.found){
                      this.fulltabledata.forEach((v) => {
                      if(this.form.fabnum == v.fabnum){
        ...
        getUser(){
           ...
           if (localStorage.getItem('fabtabledata')) {
              this.fulltabledata = JSON.parse(localStorage.getItem('fabtabledata'))
           } else {
           // 如果 localStorage 中没有数据，则将当前数据写入 localStorage
               localStorage.setItem('fabtabledata', JSON.stringify(this.tableData))
